...
<div className="center my-12 text-center">
	<div className="p-2">![Thoughts](./images/thoughts/hero.png)</div>
	Image generated using K1's unconditional dsitribution.
</div>

I usually write about my small projects. I've had a lot of unorganised thoughts and writing it down helps me articulate them.
Also, it's been around a year since I've graduated, worked at two startups, and launched my first model. I thought it's about time to reflect on what how things are going.
...
The best thing is that even if the current model training paradigm becomes obsolete the next day, any investment you have made to your data and data pipeline will be still useful.
...
Simplicity matters. Simple things work. Simple ideas permeate better. Most big advancement in ML came from simple ideas.
...
2. His paper requires no more than basic undergraduate mathematics.
3. His method is simple to implement.
4. His models actually work.

A good example of this paper are [Residual Networks](https://openaccess.thecvf.com/content_cvpr_2016/html/He_Deep_Residual_Learning_CVPR_2016_paper.html),  [PReLU](https://arxiv.org/abs/1502.01852) and
...
Most recent works include [Did we need timestep conditioning?](https://arxiv.org/abs/2502.13129), [Transformers without normalisation](https://openaccess.thecvf.com/content/CVPR2025/papers/Zhu_Transformers_without_Normalization_CVPR_2025_paper.pdf), and [Meanflow](https://arxiv.org/abs/2505.13447).

For instance, there are many papers that propose few step diffusion models at pretraining stage. 
...
I'd encourage people to read both sCM and IMM and compare it to Meanflow paper. Meanflow paper only requires basic multivariable calculus, generalises flow matching models, and is much simpler to implement.
...
Flow matching is another excellent example. Let's be honest. Diffusion and score matching are not the easiest ideas to wrap your head around. Nor it was trivial to implement.
If I'm given a blank sheet of paper and asked to fully reproduce EDM paper, I'm not too confident that I can do it. But, if it was flow matching, I'd be pretty confident.
...
Again, at the end of the day, diffusion, flow matching, score matching, ... have an unified framework and that's what makes it so beautiful. 
However, when it comes to making things work, we need to chose a method that has minimal room for error.

#### Image editing

Let's take another example. Image editing. 
There have been tones of research done on image editing quite recently, and we should reflect on what method ended up being most effective.
...
While these papers were interesting and novel, it seemed too complicated to me. In the end, the kinds of method that won out were [Kontext](https://arxiv.org/abs/2506.15742), [Bagel](https://github.com/ByteDance-Seed/Bagel), and [UNO](https://arxiv.org/abs/2504.02160).
...
<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/kontext.png)</div>
...
GPUs to train models are not cheap. However, what I have learned over the last few years is that simple, elegant methods ends up being the winners.

#### Alec Radford

When I think about Alec Radford's works, many of his core works follow a simple design. CLIP, GPT, Whisper are all simple methods that leveraged transformers and large quantity of data.
There are a lot of lessons to be learned when it comes to development of CLIP. CLIP checks off almost all the boxes I find desirable in a research project.
...
Unlocking new capability is not only exicting, but also favorable from product a standpoint. Generative model is one of the most competitive industry in the world. Quality and performance are no longer enough to simply stay in the game.
You model and product truly needs something unique that no other competitor has.
...
Debugging ML code and making fundemental improvements in research is not a difficult problem. 
...
Making things fast is desriable from research perspective, but we have seen how being "fast" has unlocked new interactions and primitives.

Programming is a good example of this. If LLM inference was slow, we would have not seen an explosion of AI code editors and tools. Certain interactions are only possible with fast inference.
...
Furthermore, because of stocastic nature of generative models, iteration is a core part of the user experience.

Perhaps, Flash Attention was the first piece of technology that made sparked the community to think more about efficiency.
I personally follow most paper that comes out of [Hazy Research](https://hazyresearch.stanford.edu/) [^5]. 
...
During my undergraduate days, a consistent pattern that I have found with many PhD students is that they don't write clean code. Research code is extremely messy.
...
This is not the case in recent times. Good presentation, usability, and code quality is a requirement if you want to get your work cited and used by the community.
ML research moves extremely fast and having an ability to understand and implement new methods has become critical. 
This is one of the reasons why I'm very very opposed to premature abstraction unless certain method has been rigorously tested and proven to be useful.

I believe this point has been iterated by many people, but I strongly believe that a __good researcher__ have to be a __good engineer__.
...
While ideas are important, the zeitgeist of the last decade has been scaling model and scaling data. We need good distributed system designs and engineering to keep scaling our models.

<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/dota.png)</div>
	DoTA 2 distributed RL system.
</div>

Among many of OpenAI's works, I still find [DoTA 2 paper](https://cdn.openai.com/dota-2.pdf) to be one of the most impressive ones. It used **a single layer 4096 unit LSTM with PPO** that was enough to beat the best human players.
...
I've given CLIP as an example above. But, if you think about it, the history of aritificial intelligence and neural network is a testament to the value of persistence.

A more recent example is diffusion models. Diffusion models in its earliest form goes back to 2015 with [Sohl-Dickstein's work](https://arxiv.org/pdf/1503.03585).
And it took many years until the community took it seriously. 
A lot of these early research efforts were spearheaded by Stefano Ermon's group and Johnathan Ho's efforts[^1]. 
...
Nowadays, everything moves so fast and research cycle has become infused with product cycle. I often complain about university and academia, but I truly believe that universities are the right place where long term research can thrive.
It's the place where multiple disciplines can come together and students have time to learn the fundementals.

<div className="my-24"/>

## Changing the objective

Many people praise transformer architecture as the secret sauce that made modern deep learning possible. 
...
<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/gpt.png)</div>
</div>
...
<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/mae.png)</div>
</div>
...
<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/simclr.png)</div>
</div>
...
<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/clip.png)</div>
</div>

The innovations that led to ChatGPT are also in changing the objective function. 
Instead of asking the model to predict the next token, you can ask the model to produce a response that maximises an arbitary reward function.

<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/rlhf.png)</div>
</div>

The core theme that appears again and again is how simple the prediction objective is. 
More importantly, none of the objective function forces an inductive bias. 
...
## How mathematics do you need for ML research?

You need it as much as you need it. A good grasp on linear algebra and multivariable calculus is needed.
Probability theory and statistics are also useful to have. Anything beyond that I find it optional depending on the research topic. Just learn it as you need it.
For diffusion models, some mathematical maturity was needed, but with introduction of flow matching, the barrier to entry has been lowered.

<div className="my-24"/>

## Boostrapping research
...
CLIP model can be used to come up with a better data to train your next CLIP. LLMs can be used to generate better synthetic data. [GAN discriminator can be used to get better GAN data](https://gwern.net/face#faces-preparation).
...
We are still at a very experimental stage of generative models. We are seeing early application of how generative models can unlock new interactions and primitives.
...
<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/right2left.png)</div>
...
Moreover, a naive implementation occuldes the model from seeing the later part of the code. Another feedback that was given at this stage was that accepting partial result was not possible.
...
<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/partial.png)</div>
...
<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/multiline.png)</div>
	Cursor's "multiline edit" feature.
</div>

Edit and completion are interesting by themselves, but programmer often needs to navigate between different parts of the code. This is where the "jump" primitive comes in.
ML engineering and research naturally blends together to power these primitives [^7] [^8] [^9].

<div className="center my-12 text-center">
	<div className="p-2 w-2/3">![Thoughts](./images/thoughts/jump.png)</div>
	Cursor's "jump" feature. It navigates you to the location of the code you want to edit next.
</div>

Product provides source of inspiration for pratical research directions, but also serves as a source of training environment. In context of code assistants, acceptance rate can be used as a proxy for reward function.
...
A lof of generative model applications still lack the level of personalisation that existing recommendation systems have. Ideally, you don't have to write a long list of rules to guide the completion of the assistant.
It should be able to infer your preference from seeing the code you write.

One of the reasons why I enjoy working at [Krea](https://www.krea.ai/) is that it's one of the very few companies where we can jointly design the product and research. I'm quite exicted to build out generative models that better understand user preferences.
**Older recommendation systems simply "retrieved" existing content we think that the user would like. Now, imagine if having a model that can "generate" content to the user's liking on the fly.**
...
It's no surprise that Google and Meta are the ones that have one of the best research teams. They are companies that already have a infinite money printer and expertise in building out large scale distributed data systems.
...
[^3]: she was initial pioneer in using masked prediction for image generation and [major contributor to gpt-4o image generation](https://x.com/SuhailAhmad/status/1939987911555825806).
