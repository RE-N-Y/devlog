...
I want to thank Google's Tensor Research Compute (TRC) program for providing compute resources for this project.

### ViT-VQGAN

![training](/devlog/images/training.png)
...
![compression](/devlog/images/compression.png)
...
One can observe that higher the compression rate there is a significant loss in quality. For instance, compare 16x16 compression against 32x32. 16x16 compression mostly captures the overall structure of the image but loses finer details in comparison to 32x32 compression.

### MUSE

![muse](/devlog/images/muse.jpg)

Using these quantized latents, one can train a transformer model to iteratively generate images. This is the approach taken by [MASKGIT](https://arxiv.org/abs/2202.04200), [MUSE](https://arxiv.org/abs/2301.00704), [Paella](https://github.com/dome272/Paella), and [MAGE](https://arxiv.org/abs/2211.09117).
Above figure (taken from MUSE paper) provides a schematic overview of the model.

![muse](/devlog/images/muse.png)

By leverging ViT-VQGAN 16x16 models, I was able to train a relatively small (~300M parameter) MUSE model at 256px resolution on TPUv3-8 hardware.
...
![stylegan](/devlog/images/stylegan.png)
...
Above are trained models from custom Stylegan implementation that I wrote a couple of years ago. I may try to reproduce results from [GigaGAN](https://mingukkang.github.io/GigaGAN/) paper to explore this direction.

### Diffusion models

![diffusion](/devlog/images/edm.png)

For diffusion models, I've implemented methods from [EDM](https://arxiv.org/abs/2206.00364) paper to generate images on 64x64 resolution. As a transfomer heretic, I've used a ViT backbone for training the diffusion model.
...
Again, I would like to thank Gwern, hysts, and nagadomi for their amazing work and contributions to the community. I hope this dataset will be useful for future anime generation research.
